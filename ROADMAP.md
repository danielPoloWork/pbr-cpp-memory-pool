...
**Status check:** As of 2026-06-11 — Milestones 0 and 1 complete (`v0.1.0` published); Milestone 2 in flight, items 2.1–2.7 complete (all four spec §5 C functions implemented + `Pool` RAII wrapper + Factory Method / Builder + foreign-pointer policy). Remaining in M2: 2.8 (Valgrind CI gate), 2.9 (microbenchmark vs `malloc`), 2.10 (metadata-overhead budget), 2.11 (close → `v0.2.0`).
