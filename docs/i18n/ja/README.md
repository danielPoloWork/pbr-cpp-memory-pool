...
`v1.2.0` —— **マイルストーン 9（エルゴノミクス・堅牢化・ツーリング）**、`v1.0.0` 以降最初の MINOR。凍結された公開インターフェース上への**追加的で ABI 互換**な機能群 —— デフォルトビルドで出荷されるライブラリは `v1.1.x` とバイト単位で同一です。[`std::pmr::memory_resource` アダプタ](../../../docs/adr/0042-pmr-memory-resource-adapter.md)（`PoolMemoryResource`、#107）、**オプトインのデバッグ堅牢化** —— 解放済みブロックのポイズニング、ガードワード、空きリストの safe-linking を `PBR_MEMORY_POOL_HARDENING` スイッチの背後に（[ADR-0043](../../../docs/adr/0043-opt-in-debug-hardening.md)、#109）、**カバレッジ誘導ファジングハーネス**（[ADR-0044](../../../docs/adr/0044-coverage-guided-fuzzing-harness.md)、#108）、そして**ベンチマーク拡張** —— p99 テールレイテンシ + `LD_PRELOAD` による jemalloc/tcmalloc ベースライン（[ADR-0045](../../../docs/adr/0045-benchmark-percentiles-and-external-baselines.md)、#111）を追加。各新機能はヘッダオンリー、オプトインスイッチの背後、またはテスト／ベンチ専用で、`v1.0.0` の C ABI と C++ 型は不変です。5 つの新規 ADR（0041–0045）で総数は 45 になりました。リリースノート：[`docs/releases/v1.2.0.md`](../../../docs/releases/v1.2.0.md)。より以前のバージョン：
