...
- **Explicit free list (separate metadata block — bitmap or linked-list table).** Rejected. The spec mandates the implicit layout in §4 (*"its first bytes are used to store a pointer to the next free block"*); even setting that aside, an explicit bitmap costs `ceil(block_count / 8)` bytes per pool and an explicit linked-list table costs `sizeof(size_t) * block_count`, both of which violate spec §3.2 (*"The use of internal metadata... must be minimal"*). Implicit gives zero per-block overhead while live, which is the strongest possible answer to §3.2.
