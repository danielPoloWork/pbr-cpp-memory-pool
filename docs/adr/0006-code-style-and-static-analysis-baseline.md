...
| `modernize-deprecated-headers`                          | Same dual-language rationale as `modernize-use-using`: `<stddef.h>` is mandatory in the C interop header (`<cstddef>` is a C++-only name). The check is correct for *pure-C++* code but cannot be selectively scoped to non-C-compat headers without a per-file `.clang-tidy`, which would multiply maintenance surface. |
| `clang-analyzer-cplusplus.NewDeleteLeaks`               | False positives traced through doctest's macro expansion: clang-analyzer's path-sensitive engine follows allocation chains into doctest internals and reports "leaks" at `doctest.h` line numbers even though the leak is structural to the framework, not our code. Disabling here trades a coarse-grained check for noise reduction; real leaks remain covered by the Valgrind job in ROADMAP §2.8. |
...
- `readability-identifier-naming.MacroDefinitionIgnoredRegexp` carries an exemption for macros matching `IT_D4NP_.*` — the project's include guards embed `D4NP` as a single word (mirroring the namespace and filesystem path locked by [ADR-0002](0002-adopt-cross-language-source-layout.md)) and end with a trailing underscore, both of which clang-tidy's default tokenizer rejects. The exemption is narrowly scoped to this prefix so generic macros (`PBR_FOO_BAR`) remain enforced.
- `readability-function-cognitive-complexity.Threshold` is set to **40** (default is 25). Doctest's `TEST_CASE` / `CHECK` / `REQUIRE` macros expand into substantial conditional structure; under the default threshold even modestly-sized test functions exceed the limit purely from macro inflation, with no actual cognitive load in the source. Forty is the smallest threshold at which the M1 smoke tests pass while still flagging genuinely-tangled functions.
