...
| 5 | Adapter | Implemented | `PoolAllocator<T>` satisfies the C++17 Cpp17Allocator requirements over the one-fixed-block-at-a-time pool interface, so the pool composes with every standard container through `std::allocator_traits`. Hybrid node-allocator semantics: single-element requests that fit a slot are pool-served, bulk / oversized / exhausted requests fall back to `::operator new`; deallocation routes by provenance via the new O(1) `memory_pool_owns` predicate (the ADR-0012 range check promoted to public API). Non-owning handle, pointer-identity propagation traits. | [`pool_allocator.hpp`](../../src/main/cpp/it/d4np/memorypool/pool_allocator.hpp) (header-only); the two backing C introspection functions live in [`memory_pool.h`](../../src/main/cpp/it/d4np/memorypool/memory_pool.h) / [`memory_pool.cpp`](../../src/main/cpp/it/d4np/memorypool/memory_pool.cpp). | [ADR-0018](../adr/0018-adapter-stl-compatible-allocator.md) |
...
| Pattern             | Possible application                                                                         |
|---------------------|----------------------------------------------------------------------------------------------|
| Adapter             | STL-compatible allocator over the underlying pool. *M3.3 adoption → Adopted #5.*             |
