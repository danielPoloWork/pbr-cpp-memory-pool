...
| 7 | Strategy | Planned | Pluggable thread-safety policy selecting how `alloc`/`free` synchronize: `SingleThreadedPolicy` (default, no-op — the v0.3.0 fast path), `MutexPolicy` (a `std::mutex`), `LockFreePolicy` (ABA-tagged Treiber-stack CAS). Bound **at compile time** (policy-based, not runtime-virtual) so the single-threaded build pays nothing, selected by `PBR_MEMORY_POOL_THREAD_SAFETY`. Per-thread caches deferred. `SingleThreadedPolicy` + the hosting skeleton landed in M4.2; the pluggable Mutex/LockFree policies + the macro selector arrive in M4.3. | Decision: [ADR-0020](../adr/0020-thread-safety-strategy-and-compile-time-knob.md). `SingleThreadedPolicy` in [`memory_pool.cpp`](../../src/main/cpp/it/d4np/memorypool/memory_pool.cpp) (M4.2); Mutex/LockFree + macro in M4.3. | [ADR-0020](../adr/0020-thread-safety-strategy-and-compile-time-knob.md) |
